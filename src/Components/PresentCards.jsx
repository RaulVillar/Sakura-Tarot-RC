    ...
    return (
        <div className="sakuraCards-box">{
            shuffleArray(data).map((item) => (
                <img onClick={() => {
                    present = [] ? (present.push(item[0].id)) && (document.getElementById(item[0].spanishName).src = item[0].sakuraCard)
                        && (document.getElementById("yourPresent").innerHTML = item[0].meaning) : present = []
                }}
